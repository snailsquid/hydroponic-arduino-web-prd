...
function initializeCharts(initialData = []) {
    ...
    console.log('Attempting to find canvas with ID "distanceChart":', canvasDistance);
    if (!canvasDistance) {
        console.error("Critical: Canvas element 'distanceChart' not found! Chart cannot be initialized.");
    ...
    if (!canvasTDS) {
        console.error("Critical: Canvas element 'tdsChart' not found! Chart cannot be initialized.");
        return; // Exit the function if canvas not found
    }
    ...
    if (!ctxDistance || !ctxTDS) {
        console.error("Could not get 2D context for one or both canvases. Charts cannot be initialized.");
        return;
    }
    // ------------------------------------
...
async function clearGraphsAndData() {
    ...
    if (!confirm('Are you sure you want to clear ALL historical data and reset the graphs? This action cannot be undone.')) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to clear data on server: ${response.status} - ${errorText}`);
        }
...
if (clearGraphsButton) {
    ...
    console.error("Error: Clear Graphs button with ID 'clearGraphsButton' not found in HTML!");
}
